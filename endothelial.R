...
# marker plots
gene <- 'Mki67'
plot(reducedDim(sce,'umap.endo')[ind,], asp=1,
...
### what are clusters 3 and 6?


# Differential Expression
# endo3,endo6 vs. all others
clus <- sce$clus.endo %in% c('endo3','endo6')

de <- scran::scoreMarkers(sce, groups = clus, block = sce$sample, assay.type = 'logcounts')
de <- de$`TRUE`
...
# Violin plots
plotExpression(sce, exprs_values = "logcounts",
               features = head(rownames(de[order(de$mean.AUC, decreasing = TRUE), ])), 
               x="clus.endo", colour_by="clus.endo")
